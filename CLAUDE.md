...
## First Steps
- **Always read `re/prd.md` before starting any work** — it contains the project scope, milestones, known challenges, and security rules.
- Check open backlog tasks: `backlog board --plain`

## Project Structure
- `re/` — All reverse engineering artifacts (analysis docs, scripts, decompilation notes)
- `secrets/` — Tokens, captured API responses, PII (gitignored, never committed)
- `linkedin/` — Rust workspace (`linkedin-api` and `linkedin-cli` crates)

## Backlog Rules
- Any new avenue, tangent, or idea discovered during work MUST be captured as a new backlog task
- One task per sub-agent, sequential execution
- Git commit after each completed task
- Do NOT chase tangents inline — create a backlog task instead

## Security Rules
- Tokens, PII, and credentials go in `secrets/` (gitignored)
- Never commit APK files, decompiled output, or captured responses
...
- Run `just e2e` before every commit (once Justfile exists)
- Run `just showcase` as regression gate (once CLI exists)

## LinkedIn-Specific Notes
- Package name: `com.linkedin.android`
- LinkedIn uses Rest.li (not standard REST) — watch for `X-RestLi-*` headers
- Auth likely involves OAuth2 + CSRF (`jsessionid`) + `li_at` cookie
